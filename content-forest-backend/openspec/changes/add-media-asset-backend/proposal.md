...
- 媒体作为枝化输入时进入当前枝化生长的授权范围、Reference Planner、ReferenceAtom 和 planned/actual usage 体系，而不是绕过现有引用规划单独传递。
...
- `branch-growth`: 枝化生长请求支持媒体引用和用途说明，并将媒体引用纳入引用规划、参考原子和实际使用摘要。
...
- SQL 契约：新增 `docs/sql/media.sql`，按需更新 `docs/sql/fruit.sql`、`docs/sql/growth.sql`，并基于当前 `growth` 契约中的 reference plan、reference atom、planned/actual usage 结构增量扩展。
