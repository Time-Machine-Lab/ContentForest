...
### Requirement: OpenClaw 作为非小红书平台营养研究默认 Provider
**Reason**: 非小红书平台需要可控的平台 API/MCP 数据采集，OpenClaw 外部 Agent 委托链路不再作为默认事实来源。
**Migration**: 非小红书平台请求迁移到 TikHub MCP Provider；Codex external research 仅作为覆盖不足或深度分析时的补盲层。

### Requirement: 旧搜索 API 作为非小红书平台营养研究默认 Provider
**Reason**: Brave、Tavily、SerpApi、PublicWebSearch、XCrawl 类路径只能提供候选网页或增加维护成本，不能稳定提供平台原生帖子详情与互动数据。
**Migration**: 非小红书平台默认通过 TikHub MCP 执行平台搜索和详情采集；旧 Provider 从默认营养研究链路移除。
