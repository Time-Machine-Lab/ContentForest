# Seed Management Spec

## Purpose

种子（Seed）是内容森林系统的核心数据实体，是所有生成器、果实池和营养汲取模块的数据来源。本规格定义种子的完整生命周期管理能力，包括创建、查询、更新、状态流转和删除。种子具有三种状态：`draft`（草稿）、`active`（活跃）、`archived`（归档）。
...
- **THEN** 系统生成新 ID（格式：`seed_{YYYYMMDD}_{nanoid(8)}`）
...
- `draft` → `active`：通过 `POST /api/seeds/publish` 触发
- `active` → `archived`：通过 `PUT /api/seeds/{seedId}/archive` 触发
- `archived` → `active`：通过 `PUT /api/seeds/{seedId}/restore` 触发
- **禁止**：`active` / `archived` → `draft`（不可逆）
- **禁止**：通过 `PATCH /api/seeds/{seedId}` 修改状态
...
| Method | Path | 描述 |
|--------|------|------|
| `POST` | `/api/seeds/draft` | 保存草稿（新建或更新） |
| `POST` | `/api/seeds/publish` | 发布种子（新建或从草稿升级） |
| `PUT` | `/api/seeds/{seedId}/archive` | 归档种子 |
| `PUT` | `/api/seeds/{seedId}/restore` | 回档种子 |
| `GET` | `/api/seeds` | 查询种子列表（支持过滤与分页） |
| `GET` | `/api/seeds/{seedId}` | 查询种子详情 |
| `PATCH` | `/api/seeds/{seedId}` | 更新种子内容信息 |
| `DELETE` | `/api/seeds/{seedId}` | 物理删除种子 |
| `GET` | `/api/tags` | 查询全部标签 |
