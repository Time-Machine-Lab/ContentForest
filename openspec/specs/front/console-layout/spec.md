...
#### Scenario: 切换主视图
- **WHEN** 用户点击侧边栏中的可用导航菜单项
- **THEN** 主内容区切换到对应视图
- **AND** 点击的菜单项显示激活态（accent bar + 背景色）

#### Scenario: 点击占位菜单项
- **WHEN** 用户点击标注 `// soon` 的菜单项
- **THEN** 该菜单项不响应点击（`opacity-40 cursor-not-allowed`）

---

### Requirement: 控制台顶部 Header
控制台主内容区顶部 SHALL 提供 Header 栏（高度 `h-14`，`sticky top-0`）。Header 左侧 SHALL 显示面包屑路径（`// Console / 当前页面名称`）。Header 右侧 SHALL 根据当前视图动态显示上下文操作按钮（种子库视图显示「+ 新建种子」）。
