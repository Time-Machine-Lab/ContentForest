...
- [x] 6.1 端到端测试：上传生成器 → 出现在市场 → 其他用户安装 → 本地 Skill 文件存在
- [x] 6.2 端到端测试：卸载生成器 → 本地文件删除 → Redis 记录清理 → installCount 不变
- [x] 6.3 在 AI IDE 中完整执行一次生成流程：选择种子 → 选择生成器 → Agent 执行编排 → 生成日志写入
