...
test('nutrient workbench dialog has three panes and narrow screen tabs', () => {
  ...
  assert.equal(dialog.includes('cf-nutrient-session-rail'), true)
  assert.equal(dialog.includes('cf-nutrient-agent-panel'), true)
  assert.equal(dialog.includes('cf-nutrient-asset-rail'), true)
  assert.equal(dialog.includes('cf-nutrient-workbench-tabs'), true)
  assert.equal(dialog.includes('cf-nutrient-workbench-stats'), false)
  assert.equal(dialog.includes('cf-nutrient-new-session-btn'), true)
  assert.equal(dialog.includes('cf-nutrient-session-draft'), false)
  ...
  assert.equal(dialog.includes('cf-nutrient-session-strip'), false)
})
...
test('nutrient workbench exposes card lifecycle actions and dependency restore state', () => {
  ...
  assert.equal(dialog.includes('deleteSelectedCard'), true)
  assert.equal(dialog.includes('requestDeleteSelectedCard'), true)
...
test('nutrient research chat loads sessions and submits messages', () => {
  ...
  assert.equal(dialog.includes('loadResearchSessions'), true)
  assert.equal(dialog.includes('selectResearchSession'), true)
  assert.equal(dialog.includes('createResearchSession'), true)
  assert.equal(dialog.includes('startNewResearchSession'), true)
  assert.equal(dialog.includes('enterNewSessionDraft'), true)
  assert.equal(dialog.includes('requestDeleteResearchSession'), true)
  assert.equal(dialog.includes('deleteResearchSession'), true)
  assert.equal(dialog.includes('window.confirm'), false)
  assert.equal(dialog.includes('nutrientCardId'), false)
  assert.equal(dialog.includes("selectedCard.value = null"), true)
  assert.equal(dialog.includes('新会话'), true)
  assert.equal(dialog.includes('bindCardConversation'), false)
  ...
  assert.equal(dialog.includes("event.type === 'tool_call_started'"), true)
  assert.equal(dialog.includes("event.type === 'tool_call_completed'"), true)
  assert.equal(dialog.includes("event.type === 'tool_call_failed'"), true)
  ...
  assert.equal(dialog.includes("event.type === 'cancelled'"), true)
  ...
  assert.equal(dialog.includes('upsertToolMessage'), true)
  assert.equal(dialog.includes('ResearchMessageView'), true)
  assert.equal(dialog.includes('createOptimisticResearchTurn'), true)
  assert.equal(dialog.includes("localStatus?: 'pending' | 'failed' | 'cancelled'"), true)
  assert.equal(dialog.includes("localKind?: 'thought' | 'assistant_stream' | 'tool' | 'progress'"), true)
  ...
  assert.equal(dialog.includes('cancelResearchMessage'), true)
  assert.equal(dialog.includes('cf-nutrient-thought-toggle'), true)
  assert.equal(dialog.includes('cf-nutrient-tool-body'), true)
  assert.equal(dialog.includes('cf-nutrient-send-btn'), true)
  assert.equal(dialog.includes('is-pausing'), true)
...
test('nutrient research chat renders depositable blocks with actions', () => {
  ...
  assert.equal(dialog.includes('mergeBlockIntoTargetCard'), true)
  assert.equal(dialog.includes('openMergeTargetSelector'), true)
  ...
  assert.equal(dialog.includes('cf-nutrient-deposit-dock'), true)
  assert.equal(dialog.includes('saveActiveDepositableBlock'), true)
  assert.equal(dialog.includes('createAllVisibleBlocks'), true)
  assert.equal(dialog.includes('mergeActiveDepositableBlock'), true)
  assert.equal(dialog.includes('locateActiveDepositableBlock'), true)
  assert.equal(dialog.includes('depositableBlockDomId'), true)
  assert.equal(dialog.includes('保存为卡片'), true)
  assert.equal(dialog.includes('保存选中'), true)
  assert.equal(dialog.includes('保存全部'), true)
  assert.equal(dialog.includes('合并到内容'), true)
  assert.equal(dialog.includes('合并选中'), true)
...
test('nutrient workbench accepts and ignores nutrient suggestions', () => {
  ...
  assert.equal(dialog.includes('suggestionPopoverOpen'), true)
  assert.equal(dialog.includes('cf-nutrient-suggestion-trigger'), true)
  ...
  assert.equal(dialog.includes('enterNewSessionDraft(suggestion.bodyMarkdown)'), true)
  assert.equal(dialog.includes('cf-nutrient-suggestion-card'), true)
  assert.equal(dialog.includes('cf-nutrient-suggestion-popover'), true)
  assert.equal(dialog.includes('suggestionSourceLabel'), true)
})

test('nutrient workbench decouples sessions from nutrient assets', () => {
  const dialog = readProjectFile('app/components/nutrient/NutrientWorkbenchDialog.vue')
  const nutrientApi = readProjectFile('src/modules/nutrient/api.ts')
  const nutrientTypes = readProjectFile('src/modules/nutrient/types.ts')

  assert.equal(dialog.includes("type WorkbenchMode = 'session' | 'new-session' | 'card-detail'"), true)
  assert.equal(dialog.includes('activeWorkbenchMode'), true)
  assert.equal(dialog.includes('cardSearch'), true)
  assert.equal(dialog.includes('cardStatusFilter'), true)
  assert.equal(dialog.includes('mergeTargetSearch'), true)
  assert.equal(dialog.includes('mergeTargetStatusFilter'), true)
  assert.equal(dialog.includes('mergeTargetSurface'), true)
  assert.equal(dialog.includes('cf-nutrient-inline-confirm'), true)
  assert.equal(dialog.includes('cf-nutrient-delete-confirm'), true)
  assert.equal(dialog.includes('cf-nutrient-composer-field'), true)
  assert.equal(dialog.includes('研究输入'), false)
  assert.equal(dialog.includes('<select'), false)
  assert.equal(dialog.includes('cf-nutrient-segmented-filter'), true)
  assert.equal(dialog.includes('grid-auto-rows: max-content'), true)
  assert.equal(dialog.includes('汲取消息'), true)
  assert.equal(nutrientApi.includes('deleteResearchSession'), true)
  assert.equal(nutrientApi.includes('deleteCard'), true)
  assert.equal(nutrientApi.includes('mergeCard'), true)
  assert.equal(nutrientTypes.includes("export type NutrientWorkbenchPane = 'sessions' | 'agent' | 'cards'"), true)
})
