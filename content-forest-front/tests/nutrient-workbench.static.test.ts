...
test('nutrient research chat loads sessions and submits messages', () => {
  ...
  assert.equal(dialog.includes("event.type === 'thought_delta'"), true)
  assert.equal(dialog.includes("event.type === 'message_delta'"), true)
  assert.equal(dialog.includes("event.type === 'nutrient_block_delta'"), true)
  ...
  assert.equal(dialog.includes('appendResearchMessageDelta'), true)
  assert.equal(dialog.includes('appendStreamDepositableBlock'), true)
