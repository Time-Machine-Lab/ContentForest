...
前端 SHALL 将媒体资源作为工作区可引用资源类型集成到枝化生长输入框中。该能力 MUST 与营养库、基因库引用共存，MUST 不破坏现有 `@` 引用和资源移除能力。
...
- **WHEN** 用户同时引用营养、基因和媒体资源
- **THEN** 前端 MUST 同时展示这些引用
- **AND** 提交生长任务时 MUST 分别映射 nutrientRefs、geneRefs 和 mediaRefs
