...
- **联网数据受控获取**：联网研究必须通过内容森林封装的 Provider Router。当前默认实现为外部 Agent Provider 链路，优先可接 OpenClaw，失败、超时或无可用结果时降级 Codex；内容森林只提供研究指令、输出契约、归一化与 Trace，Skill 不允许直接绑定外部搜索服务、浏览器命令或平台爬取实现。
...
联网数据获取 Tool 底层使用 Provider Router，而不是绑定单一搜索服务。当前默认链路是 OpenClaw 外部 Agent Provider 优先、Codex 外部 Agent Provider 兜底：内容森林把用户研究目标、目标平台、期望结果数量、不可编造约束和输出契约组装成研究指令，外部 Agent 自主完成联网搜索、必要的页面观察、资料判断和初步总结，后端只负责结构化校验、归一化、去重、新鲜度标记和排序，最终将研究上下文包交给营养研究 Skill 总结。

旧的本地双层搜索浏览能力保留为非默认实现：

- **Search Provider**：`ConfiguredSearchApiProvider` 和 `PublicWebSearchProvider` 代码保留，但默认关闭，后续只在显式配置时用于调试或降级。
- **Browser Action Runtime + Platform Browser Strategy**：`BrowserResearchProvider` 默认深度探索和平台专项 Provider 代码保留，但默认关闭，后续只在显式配置时用于受控浏览器观察或平台专项验证。

OpenClaw 外部 Agent Provider 使用 Gateway 会话执行研究任务，Provider 必须为每次调用创建独立 session，并在调用结束后由系统基础设施层删除 session，删除动作不得暴露给 Agent。Codex 外部 Agent Provider 使用 Responses 兼容接口作为降级链路。Provider 必须要求外部 Agent 返回结构化研究结果，并在 Trace 中记录 Provider 顺序、降级路径、结果数量和失败原因；鉴权 Token、Gateway 私密信息和 session 管理能力不得进入 Agent 可见上下文。

联网研究结果必须表达来源质量：外部 Agent 返回的资料默认是候选线索，只有当其明确提供可追溯页面观察证据时才可升级为已观察案例。验证码、登录墙、访问受限、布局变化、空结果和 Provider 不可用都属于受限状态，Agent 只能如实说明限制，不能把受限页面或候选线索包装成真实案例。
