...
第二期枝化生长调用会额外携带本轮生长简报、搜索模式、突变激进程度、内容解空间摘要、平台推断来源、attempt 级选中探索路线、参考计划、突变算子和突变计划。AgentPort 只接收这些由业务模块编排出的任务语义，不负责维护长期简报或系统事实，也不得因为路线文本中出现资源名称就扩大授权范围。Agent 执行过程中可以通过专门的用户进度事件上报生成路径子步骤，后端应用服务负责把这些用户进度事件汇总到任务状态响应中。普通 Agent Trace 仍然只服务工程调试，不直接展示给用户。
...
- 本次枝化生长的解空间摘要、平台推断来源、选中探索路线、参考计划和突变算子是什么。
- 本次任务禁止访问哪些范围外内容。

Agent 调用 Tool 时必须受任务上下文约束。未被授权的资源，即使物理上存在，也不能被 Agent 读取。

路线元数据只是策略输入，不是授权清单。即使选中路线、证据计划或参考计划提到某类资源、平台案例或内容线索，Agent 仍只能通过任务授权范围读取来源节点、生成器、正式营养、临时营养卡片、基因和反馈上下文。Tool 授权必须继续以 `authorizationScope` 为准，而不是以路线文本为准。
...
内置枝化生长 Skill 在第二期必须优先使用选中探索路线和参考计划来组织生成器 payload 输入。固定探索槽位只能作为缺少有效路线时的兼容兜底，并且需要在 Trace 中记录。证据卡片应覆盖来源节点、正式营养、临时营养卡片、基因和反馈语义；正式营养表达为已沉淀参考，临时营养卡片表达为候选或低置信参考。外部生成器 Skill 可以接收路线、参考计划和突变算子作为 payload 生成输入，但仍只返回 payload，不创建果实，不声明系统事实。
