...
- **联网数据受控获取**：联网研究必须通过内容森林封装的 Provider Router。第二期默认实现为平台证据管线：目标规划、平台路由、能力判断、候选发现、详情补全、覆盖率/质量门控、营养综合。小红书请求优先使用 `xiaohongshu-cli` Provider 获取可复查帖子证据；除小红书外的 TikHub 支持社媒平台优先使用 TikHub MCP Provider 获取平台原生证据；Codex external research 仅在门控后补盲或深研。Skill 不允许直接绑定外部搜索服务、浏览器命令、平台爬取实现、登录态或 Provider 私有配置。
...
第二期枝化生长调用会额外携带本轮生长简报、搜索模式、突变激进程度、内容解空间摘要、平台推断来源、attempt 级选中探索路线、参考原子摘要、参考计划、参考使用摘要、突变算子和突变计划。AgentPort 只接收这些由业务模块编排出的任务语义，不负责维护长期简报或系统事实，也不得因为路线文本中出现资源名称就扩大授权范围。Agent 执行过程中可以通过专门的用户进度事件上报生成路径子步骤，后端应用服务负责把这些用户进度事件汇总到任务状态响应中。普通 Agent Trace 仍然只服务工程调试，不直接展示给用户。
...
- 本次枝化生长的解空间摘要、平台推断来源、选中探索路线、参考原子、参考计划、计划/实际参考使用摘要和突变算子是什么。
- 本次任务禁止访问哪些范围外内容。

Agent 调用 Tool 时必须受任务上下文约束。未被授权的资源，即使物理上存在，也不能被 Agent 读取。

路线元数据、参考原子和参考计划只是策略输入，不是授权清单。即使选中路线、证据计划或参考计划提到某类资源、平台案例或内容线索，Agent 仍只能通过任务授权范围读取来源节点、生成器、正式营养、临时营养卡片、基因和反馈上下文。Tool 授权必须继续以 `authorizationScope` 为准，而不是以路线文本或参考原子文本为准。
...
当外部生成器 Skill 通过受控工具产出图片、视频或其他媒体文件时，Agent Runtime 只能把这些结果登记为候选媒体产物摘要。摘要可包含来源工具、MIME、大小、展示角色、是否必要和接管所需的内部临时内容句柄，但不得把本机绝对路径、临时文件位置或前端可访问地址返回给业务响应。正式 Media Asset 的创建、内容保存、数据库事实写入和果实挂载只能由后端应用服务在结果封装阶段完成。

联网数据获取 Tool 底层使用 Provider Router，而不是绑定单一搜索服务。第二期默认链路按证据管线拆分：

1. **Goal Planner**：从用户自然语言中提取平台、内容对象、目标数量、案例意图和深研需求，并清理任务噪声。
2. **Platform Router**：基于平台别名和请求语义选择确定性 Provider。小红书别名包括“小红书、xhs、xiaohongshu、rednote”；TikHub 非小红书平台包括 TikTok、抖音、Instagram、微博、B 站、YouTube、快手、知乎、LinkedIn、Reddit、微信、Twitter/X、Threads 等。
3. **Capability Gate**：根据平台能力矩阵判断是否存在安全可用的搜索、详情、评论、作者和互动指标工具；需要平台 Cookie、账号私域权限或写操作权限的工具默认跳过。
4. **Candidate Discovery**：通过平台 Provider 搜索候选内容，只形成候选线索，不直接进入营养结论。
5. **Detail Enrichment**：对候选内容读取详情，补齐标题、作者、封面、正文、发布时间和互动数据。
6. **Coverage/Quality Gate**：统计目标数量、完整已观察案例、受限状态和结果缺口，决定是否触发 Codex external research。
7. **Nutrient Synthesis**：营养研究 Skill 只基于证据层级生成普通回复和可沉淀营养块，并保留来源引用。

小红书默认进入 `xiaohongshu-cli` Provider。该 Provider 由后端基础设施层负责调用本地 `xhs` CLI、检查登录态、解析结构化输出和脱敏错误摘要。非小红书社媒默认进入 TikHub MCP Provider。该 Provider 由后端基础设施层通过 Streamable HTTP MCP 调用 TikHub hosted MCP server，内部维护 MCP session id、工具列表缓存、平台别名、能力矩阵和工具映射；TikHub API key、Authorization header、MCP session id、平台 Cookie 和完整工具响应不得进入 Agent 可见上下文。Codex external research 保留为 coverage gate 之后的补盲/深研层：它可以扩展关键词、补充背景和总结打法，但不能把未实采内容升级为平台真实帖子。

旧搜索 API、公开网页搜索、浏览器页面策略和占位 Provider 不参与默认营养研究链路；`controlled_web_search` 不作为营养研究入口注册。Provider 必须在 Trace 中记录路由、健康检查、平台能力摘要、候选数量、详情读取数量、完整案例数量、受限状态、Codex 是否触发和失败原因；鉴权 Token、MCP session id、Cookie、浏览器 Profile、本地绝对路径、完整 stderr 和超长平台正文不得进入 Agent 可见上下文。

联网研究结果必须表达来源质量：只有搜索列表但未读详情的资料是候选线索；读取详情并具备可复查 ID 或链接、标题、作者、正文或正文摘要、至少一种互动数据的结果才是完整已观察案例。验证码、登录墙、访问受限、布局变化、空结果、工具不可用、平台能力不足和 Provider 不可用都属于受限状态，Agent 只能如实说明限制，不能把受限页面或候选线索包装成真实案例。
...
内置枝化生长 Skill 在第二期必须优先使用选中探索路线、参考原子和参考计划来组织生成器 payload 输入。固定探索槽位只能作为缺少有效路线时的兼容兜底，并且需要在 Trace 中记录。证据卡片应覆盖来源节点、正式营养、临时营养卡片、基因和反馈语义；正式营养表达为已沉淀参考，临时营养卡片表达为候选或低置信参考。参考原子必须按内容槽位和使用动作进入生成上下文，广告主主张、论文结论和平台观察信号不得被直接写成无条件事实。外部生成器 Skill 可以接收路线、参考计划和突变算子作为 payload 生成输入，但仍只返回 payload，不创建果实，不声明系统事实。
...
- 路线级策略信息，包括平台推断来源、解空间摘要、选中路线标识、参考计划摘要、突变算子、成功信号、风险约束和候选封装结果。
- 参考规划信息，包括参考原子数量、来源类型摘要、风险等级摘要、slot-action 路由、provided/planned/actual 使用状态、风险处理摘要和事实检查摘要。
- 枝化生长工程运行细节，例如生成器读取、Tool 调用、LLM 调用和输出校验。
- 联网研究阶段，例如外部 Agent Provider、模型、wire API、web search 使用摘要、结果层级、受限状态、降级路径和总结阶段。
...
- **路线级用户进度**：枝化生长可上报平台推断、解空间建图、选择探索路线、执行生成器和封装候选果实等用户能理解的步骤；步骤名称不得直接暴露 tool_called、llm_called 或 skill_called 等工程事件。
...
- **联网访问**：联网数据获取通过受控 Provider Router 执行。小红书等平台证据由后端 Provider 采集并归一化，Skill 不接收 API Key、Cookie、浏览器登录态、本地 Profile 路径、CLI 路径或 Provider 私有配置。
...
- **决策**：第一期 Agent Tool 只允许读取系统事实，不允许写入、删除、修改状态或创建业务对象。生成器工具如需在受控临时目录内产生媒体文件，只能把它交还为候选媒体产物，不能直接成为系统事实。
...
- **决策**：Agent Runtime 必须坚持系统指令优先级，并通过任务授权、只读 Tool、参考原子边界和结构化输出校验限制外部文本对系统行为的影响。
- **理由**：内容森林会大量读取用户和外部资料，必须把不可信内容视为数据，而不是系统指令。
- **后果**：Agent Prompt、Tool 调用、参考计划和输出校验都需要体现安全边界，不能把外部内容当成可执行系统命令；资料中出现“忽略规则”“读取路径”“写入文件”等文本时，只能作为资料内容处理。
