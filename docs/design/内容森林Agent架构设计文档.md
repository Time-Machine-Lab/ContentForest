...
业务模块向 AgentPort 发起枝化生长请求，并提供本次任务授权范围。Agent Runtime 在授权范围内通过 Tool 读取种子或果实内容、生成器 Skill、营养资料和基因库经验，再通过内置枝化生长 Skill 编排生成器 Skill 与 LLM 能力，最终返回候选果实、候选内容和基因标签建议。Agent 不创建果实，不写内容文件，不维护内容树关系。
...
- **枝化生长 Skill**：负责将种子/果实、用户输入、生成器、营养、基因库经验等上下文组织为内容生长任务，并将生成器 payload 整理为候选果实结果。
- **基因汲取 Skill**：负责基于果实、基因标签、选择结果、发布验证和数据反馈生成基因经验建议。
