...
联网数据获取 Tool 底层使用 Provider Router，而不是绑定单一搜索服务。Provider 可以包括 Search Provider、网页正文抓取、Browser Action Runtime 和平台专项数据源。营养汲取使用研究模式：先通过查询规划器拆分用户请求，再归一化、去重、新鲜度标记和排序，最终将研究上下文包交给 Agent 总结。

联网研究在 Agent 层分为两段执行：

- **初步搜索层**：由 Search Provider 执行，优先接入 Brave、Tavily、SerpApi 等 API 型搜索服务，负责广度发现候选线索。Search Provider 与浏览器能力解耦，Key 缺失、额度不足、网络失败都必须结构化返回。
- **深入探索层**：由 Browser Action Runtime + Platform Browser Strategy 执行。Browser Action Runtime 只负责受控打开、读取、输入、提交和快照等浏览器动作；Platform Browser Strategy 负责不同平台的入口、站内搜索、结果读取、详情读取和受限状态识别。

浏览器观察 Provider 第一版通过 `agent-browser.dev` CLI 访问真实网页。它只用于深入探索和指定链接观测，不作为默认搜索入口；小红书策略应优先访问小红书站点或小红书候选链接，而不是打开搜索引擎网页 UI。它必须使用任务级 session，禁止多个任务共享默认 session；同一 session 内操作串行执行，不同任务通过全局并发池限制同时运行数量。浏览器 Provider 必须受域名白名单、最大步骤数、超时时间、截图数量和返回正文长度约束。

联网研究结果必须表达来源质量：Search Provider 返回的是候选线索，Browser Action 打开并读取平台页面后才是已观察案例。验证码、登录墙、访问受限、布局变化、空结果和 Provider 不可用都属于受限状态，Agent 只能如实说明限制，不能把受限页面或候选线索包装成真实案例。
...
- 联网研究阶段，例如查询规划、初步搜索 Provider、深入探索触发原因、平台策略、Browser Action 步骤摘要、结果层级、受限状态、降级路径和总结阶段。
