...
- **联网数据受控获取**：联网研究必须通过内容森林封装的 Provider Router。第二期默认实现为平台证据管线：目标规划、平台路由、候选发现、详情补全、覆盖率/质量门控、营养综合。小红书请求优先使用 `xiaohongshu-cli` Provider 获取可复查帖子证据；Codex external research 仅在门控后补盲或深研。Skill 不允许直接绑定外部搜索服务、浏览器命令、平台爬取实现、登录态或 Provider 私有配置。
...
2. **Platform Router**：基于平台别名和请求语义选择确定性 Provider。小红书别名包括“小红书、xhs、xiaohongshu、rednote”。
3. **Candidate Discovery**：通过平台 Provider 搜索候选内容，只形成候选线索，不直接进入营养结论。
4. **Detail Enrichment**：对候选内容读取详情，补齐标题、作者、封面、正文、发布时间和互动数据。
5. **Coverage/Quality Gate**：统计目标数量、完整已观察案例、受限状态和结果缺口，决定是否触发 Codex external research。
6. **Nutrient Synthesis**：营养研究 Skill 只基于证据层级生成普通回复和可沉淀营养块，并保留来源引用。

小红书默认进入 `xiaohongshu-cli` Provider。该 Provider 由后端基础设施层负责调用本地 `xhs` CLI、检查登录态、解析结构化输出和脱敏错误摘要。Codex external research 保留为 coverage gate 之后的补盲/深研层：它可以扩展关键词、补充背景和总结打法，但不能把未实采内容升级为小红书真实帖子。

旧搜索 API、公开网页搜索、浏览器页面策略和占位 Provider 不参与默认营养研究链路；`controlled_web_search` 不作为营养研究入口注册。Provider 必须在 Trace 中记录路由、健康检查、候选数量、详情读取数量、完整案例数量、受限状态、Codex 是否触发和失败原因；鉴权 Token、Cookie、浏览器 Profile、本地绝对路径和完整 stderr 不得进入 Agent 可见上下文。

联网研究结果必须表达来源质量：只有搜索列表但未读详情的资料是候选线索；读取详情并具备可复查 ID 或链接、标题、作者、正文或正文摘要、至少一种互动数据的结果才是完整已观察案例。验证码、登录墙、访问受限、布局变化、空结果和 Provider 不可用都属于受限状态，Agent 只能如实说明限制，不能把受限页面或候选线索包装成真实案例。
