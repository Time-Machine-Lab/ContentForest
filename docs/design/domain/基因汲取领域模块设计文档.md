...
- **不包含**：不负责创建种子，不负责创建果实，不负责执行物竞天择，不负责记录发布验证，不负责录入数据快照，不负责营养库资料维护，不自动改写生成器，不自动替用户确认基因，不实现复杂遗传算法，不计算统一适应度分数。
...
- **基因经验 (Gene Insight)**：用户确认后的可复用经验，可以是成功基因、失败教训、适用场景、规避规则或下一轮生长建议。
...
基因经验的正文由 Markdown 承载，系统事实由数据库维护。Markdown 只保存用户可读的经验正文，不保存归属关系、证据关系、状态、索引、生态位或内容位置等 meta 信息。
...
- **谱系轻量约束**：第一期基因谱系用于组织方向，不做复杂遗传算法、自动繁殖策略或适应度优化模型。
