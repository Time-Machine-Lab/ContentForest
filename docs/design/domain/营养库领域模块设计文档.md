...
- **平台证据 (Platform Evidence)**：联网研究 Provider 返回的外部平台资料，包含候选线索、已观察案例、完整已观察案例和受限状态。平台证据只服务营养研究会话和可沉淀营养块生成，用户保存或沉淀前不属于正式营养库内容。
...
平台证据不是营养库系统事实。小红书等平台 Provider 返回的帖子标题、作者、封面、正文摘要和互动数据，只能作为研究会话上下文与可沉淀营养块的来源引用。用户确认前，这些证据不得写入正式营养内容、公共营养库或反馈事实；用户保存为草稿后，也仍然处于种子工作台生命周期，而不是公共资料。
