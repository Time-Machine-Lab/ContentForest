...
- **包含**：创建营养库，区分公共营养库与种子专属营养库，上传营养内容，编辑营养内容，归档营养内容，回档营养内容，查看营养库与营养内容，维护种子工作台中的营养内容生命周期，设置默认带入，并为枝化生长提供可引用或临时引用的营养资料。
- **不包含**：不负责 Agent 检索策略，不负责向量知识库，不负责自动网页采集，不负责判断营养内容是否有效，不负责枝化生长，不负责参考原子化或参考路由，不负责基因汲取分析，不负责基因库维护，不负责跨用户共享市场，不直接把系统 meta 写入 Markdown。
...
- **营养研究会话 (Nutrient Research Session)**：归属于种子的 Agent 研究过程，用于承载对话、工具调用、研究消息和可沉淀营养块。它不属于营养内容身份，也不与某条营养内容绑定；会话删除不影响营养内容。
- **平台证据 (Platform Evidence)**：联网研究 Provider 返回的外部平台资料，包含候选线索、已观察案例、完整已观察案例和受限状态。小红书证据来自 `xiaohongshu-cli` Provider；非小红书社媒证据默认来自 TikHub MCP Provider；Codex external research 只能作为补盲背景、候选线索或综合分析。平台证据只服务营养研究会话和可沉淀营养块生成，用户保存或沉淀前不属于正式营养库内容。
- **可沉淀营养块 (Depositable Nutrient Block)**：Agent 基于研究会话和平台证据提炼出的候选创作营养，例如标题钩子、封面策略、正文结构、用户痛点、评论语言和互动信号。它必须保留足够来源引用，不能只是原帖堆砌。
...
graph TD
    ...
    ResearchSession["营养研究会话 / ResearchSession<br/>种子级过程"] -.产出可沉淀营养块.-> NutrientCard
...
营养研究会话与工作台营养内容相互独立。会话用于承载 Agent 研究过程、平台证据、受限状态和候选资料，营养内容用于承载可引用、可迭代、可归档的资料资产。会话可以产出多个可沉淀营养块，但只有当用户选择“保存为新草稿”或“合并到指定营养内容”时，候选资料才进入营养内容生命周期。

平台证据不是营养库系统事实。小红书 Provider 和 TikHub MCP Provider 返回的帖子标题、作者、封面、正文摘要和互动数据，只能作为研究会话上下文与可沉淀营养块的来源引用。用户确认前，这些证据不得写入正式营养内容、公共营养库或反馈事实；用户保存为草稿后，也仍然处于种子工作台生命周期，而不是公共资料。
...
- **会话解耦约束**：工作台营养内容不绑定研究会话；研究会话删除、归档或重开都不影响已经存在的营养内容。
- **平台证据候选约束**：平台证据只作为研究会话内的候选资料；用户保存为草稿或显式合并前，不进入营养内容生命周期。
- **证据层级约束**：可沉淀营养块必须区分候选线索、已观察案例、完整已观察案例和受限状态，不能把未验证线索或受限页面表述为真实平台案例。
- **实采来源分层约束**：可沉淀营养块必须区分 `xiaohongshu-cli` 小红书实采证据、TikHub MCP 非小红书社媒实采证据、Codex 候选线索和 Codex 综合分析。Codex 推断不得被表述为任一平台的原始帖子事实。
- **平台能力不足约束**：当 TikHub MCP 对某个平台缺少安全可用的搜索、详情或互动指标工具时，营养研究会话只能记录结构化受限状态，不得生成伪造的作者、正文或互动数据。
- **来源引用约束**：可沉淀营养块中用于标题钩子、封面策略、正文结构、用户痛点、评论语言和互动信号的结论，应能回溯到具体平台证据或明确说明证据不足。
- **显式合并目标约束**：会话产出的可沉淀营养块合并到既有营养内容时，必须由用户或客户端显式选择目标营养内容，系统不得根据当前会话或当前选中内容隐式推断。
...
- **参考规划边界约束**：营养内容被枝化生长引用后，可以由枝化生长注意力编排层拆分为参考原子并路由到具体内容槽位；该拆分结果只是一次生长任务的策略产物，不写回营养库系统事实，也不改变营养内容生命周期。
